...
resource "aws_wafv2_ip_set" "hub_allowlist" {
  ...
  # RECONCILIADO 2026-06-25: a migracao strangler do edge ja concluiu (o ip_set vive
  # no state do ws hub-foundation-edge; o ws de build nao tem mais recursos WAF). O
  # ignore_changes[addresses] foi REMOVIDO — a allowlist agora e gerida 100% via
  # Terraform/TFC (fonte da verdade = var.waf_allowed_cidrs no terraform.tfvars).
}
