  ...
  volume {
    ...
    efs_volume_configuration {
      ...
      authorization_config {
        access_point_id = local.data.efs_ap_alertmanager_id
        iam             = "ENABLED"
      }
    }
  }
      ...
      environment = [
        {
          ...
          value = base64encode(templatefile("${path.module}/configs/alertmanager.yml.tpl", {
            ...
            aiops_webhook_url = var.enable_alert_investigation ? "http://chainlit-chat.${var.cloudmap_namespace}:8501/api/alert-investigate" : (var.enable_aiops ? "${local.ai_platform.alert_webhook_api_endpoint}/prod/v2/webhook/alertmanager" : "")
          }))
        }
      ]
...
resource "aws_ecs_service" "alertmanager" {
  ...
  service_registries {
    registry_arn = local.network.service_discovery_services["alertmanager"].arn
  }
